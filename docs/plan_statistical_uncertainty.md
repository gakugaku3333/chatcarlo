...
作成日: 2026-07-22 / ステータス: **Phase 0-4 実装完了、本計画のスコープは完了。
ただし受入基準8（壁時間+10%以内）は事前登録された形では判定不能と判明し
（基準の設計欠陥。詳細はPhase 4節）、条件2（workers=4）のオーバーヘッドは
未決着のまま申し送る。PDD_RESULTS.md等の現在物理での再検証も別スコープの
申し送り事項** / 実行担当: Claude
...
- **`accumulate_track_length`のサブステップ方式そのものの変更**（[[future-directions]]
  ...
  を初めて切り分けられる）。**追記(2026-07-26)**: 候補3は別セッションで実装完了。
  本計画で作ったRツールを使って「解像度を細かくすると最大値が増大する」現象を
  再検証した結果、CLAUDE.mdに記載していた「max_substepsクランプ由来」という仮説は
  誤りと判明した（新旧タリー方式を直接比較すると平均値は統計誤差内で一致——
  実体はビーム幅に対するボクセルサイズの通常の空間平均化で、物理的に正しい挙動）。
  詳細は[docs/speedup_baseline/tally_exact_resolution_growth.txt](speedup_baseline/tally_exact_resolution_growth.txt)、
  CLAUDE.mdの"Known sharp edges"参照。
...
- **候補3（解析的重なり長タリー）との接続**（**完了・2026-07-26**）: 本計画のRを
  使って「解像度を細かくすると最大値が増大する」現象を再検証した。結果は
  「統計揺らぎでも系統誤差（バグ）でもなく、ビーム幅に対するボクセルサイズの
  通常の空間平均化」——CLAUDE.mdに記載していた「max_substepsクランプ由来」
  仮説は誤りだったと判明した。詳細は
  [docs/speedup_baseline/tally_exact_resolution_growth.txt](speedup_baseline/tally_exact_resolution_growth.txt)。
...
### Phase 4: 実測・検証・文書化（2026-07-26）

1. **壁時間オーバーヘッド実測**（受入基準8）。**生データは
   `docs/speedup_baseline/uncertainty_phase4_overhead.txt`に保全してある**
   （全反復の実測値・外れ値・中断した測定も含む）。
   **受入基準8は、事前登録された形
   （Phase 0で固定した絶対壁時間しきい値 条件1≤30.3s・条件2≤90.6s に対する判定）
   では成立しない**——この計測環境ではセッション間・負荷状態によって同一コードの
   壁時間が10〜27%動くため、絶対値しきい値が測定量として意味を持たない。以下、
   実測でそう判断した経緯と、代わりに何を測ったか。

   **(a) 初回の素朴な再測定（2026-07-26）と、その誤り**: 条件1 統計ON 30.26s /
   OFF 28.79s、条件2 統計ON 101.75s / OFF 101.05s。条件2をPhase 0基準82.34sと
   比べると+23.6%で基準超過に見える。ここで「同一セッション内のON/OFF差は+0.7%
   （条件1は+5.1%）だから機構コストは基準内」と結論したが、**これは各条件1回
   測定の点推定であり、後述のとおり測定ノイズ以下の数字だった**（統計不確かさを
   一級市民にする計画で、判定側が点推定というのは自己矛盾でもあった）。

   **(b) 真のベースラインを立てた対照実験**: 統計機構が存在しないコミット
   `e99a2b0`をgit worktreeに展開し、`baseline（統計コード無し）/ 現行--no-uncertainty
   / 現行統計ON`の3armを**インターリーブして3反復**した（条件1、マシンが安定状態）。

   | arm | mean | SD | 実測値 |
   |---|---|---|---|
   | baseline（`e99a2b0`、統計コード無し） | 28.06s | 0.42 | 28.51 / 27.68 / 27.98 |
   | 現行 `--no-uncertainty` | 28.76s | 0.89 | 29.76 / 28.50 / 28.03 |
   | 現行 統計ON | 28.66s | 0.34 | 29.02 / 28.34 / 28.62 |

   同一rep内のON−OFF差は **−0.74s / −0.16s / +0.59s と符号すら揃わず**、平均
   −0.10s（−0.4%）。run-to-runのばらつきは約2.0%。**統計機構のON/OFFコストは
   この環境の測定分解能以下**であり、(a)で報告した+5.1%・+0.7%はいずれも
   ノイズだった。統計ON vs 真のベースラインコードでも+2.2%（ばらつきと同程度）。
   なお同一コード（baseline）でもPhase 0記録27.57s→今回28.06sと+1.8%ずれており、
   セッション間の絶対値比較が成り立たないことが直接確認できた。

   **(c) 条件2と、計測環境そのものの限界**: 条件2（n=1e7, workers=4）を同様に
   インターリーブすると、壁時間に **432.67s・1080.23s** という桁違いの外れ値が
   出た（作業中にマシンがスリープした事象と符合。以前にも642sの異常値あり）。
   スリープに強いCPU時間(user+sys)で測り直したところ、今度は**同一のbaseline
   コードが連続する2反復で 28.83s → 36.66s（+27.2%）と持続負荷で単調に劣化**し、
   さらに1rep内の baseline→OFF→ON の単調増加が**実行順序と完全に一致**していた
   （＝熱ドリフトとarm効果が交絡している。固定順で回した計測設計の欠陥）。
   条件2については**判定に足るデータが取れなかった**。得られた範囲では
   baseline 97.30s / `--no-uncertainty` 106.05s という組があり、統計機構を
   切っても同程度に遅いことから、仮にこの差が実在するとしても統計機構起因ではない
   （`--no-uncertainty`時は`energy_moments=None`・`VoxelGrid`の追加配列も未確保で
   あることをコードで確認済み）。**条件2の+9%相当が実在の回帰かドリフトかは未決着**。

   **判定**: 統計機構自体の壁時間コストは、条件1では測定分解能以下（追加最適化は
   不要——`accumulate_track_length`のO(M×n_vox)全面走査を絞る最適化は見送り）。
   条件2は未決着として申し送る。あわせて**受入基準8の設計自体を欠陥として記録する**
   ——「別セッションで測った絶対壁時間の±10%」は、この環境では再現しない量を
   基準にしていた。今後この種の基準を書くなら、(i)同一セッション内のペア比較で
   定義する、(ii)反復回数とばらつきの許容幅を事前に決める、(iii)arm順序を反転
   させて熱ドリフトと交絡させない、の3点を基準の定義に含めること。
   教訓は[[lessons_learned]]「統計不確かさPhase 4: ベースライン比較は…」。
   - メモリ概算式（設計判断6、36バイト/voxel増分）は、グリッドメモリが支配的に
     なる細解像度条件（resolution=2cm、12,375,000 voxel）で実測37.78バイト/voxelと
     裏取り（5%以内で一致）。既定解像度（790,000 voxel）での測定は輸送側バッファの
     ノイズに埋もれるため使わなかった。なおこのOFF側の測定は、壁時間が
     スリープで汚染された実行（real 642s / user 6.17s）と同一のもの——
     ピークメモリはスリープの影響を受けないため値自体は有効だが、
     **同じ実行から壁時間を読んではいけない**。
2. **動く実例**（受入基準6・7）: chest_room.yaml（n=1e6, seed=42, resolution=5cm,
   batch-size=50000）のRマップ・鉛遮蔽（`control_booth_shield`）背後が
   z=107.5cm断面（`--axis z --pos 105`）で寄与バッチ0のグレー領域として明確に
   塗り分けられる図・n=1e6とn=1e7のR比較図をユーザーへ提示済み。
   R比較の条件と結果: **両runで寄与バッチ数>=5のボクセルに限定**した462,051個で
   R(1e7)/R(1e6)の中央値0.327、理論値1/√10=0.316。この選別は必要（寄与バッチ数が
   極端に少ないボクセルのRは推定自体が不安定）だが、**裏を返せば「よくサンプル
   された領域では1/√Nが成り立つ」ことしか示していない**——選別を外した領域での
   挙動は本図の主張範囲外。実測中央値が理論値より+3.5%大きい向きは、n=1e6側の
   低寄与ボクセルでRが楽観側に偏る（設計判断7の「必要条件であって十分条件では
   ない」）ことと整合する。
3. **検証スクリプトの置き換え**（副目標・受入基準9）: `run_chatcarlo_pdd60.py`の
   `arr_keV.std(ddof=1)/sqrt(N)`手書き計算を、1historyを1バッチとする規約
   （batch_size=1、Phase 1のブルートフォース照合と同じ根拠）でΣx・Σx²を実行時に
   蓄積し`chatcarlo.tally.standard_error`/`relative_error`へ渡す実装に置換。
   旧実装（ビンあたりN_TOTAL=5,000,000要素の全history配列を保持、約1.2GB）に対し
   メモリを実質ゼロに削減。**検証方法の学び**: チェックイン済み
   `chatcarlo_pdd60_results.json`と単純再実行を比較したところ平均値が最大2.6%
   動いて見えたが、原因は本変更ではなく、JSON最終生成（コミット`d597c66`、
   2026-07-17）以降に入った物理側の変更（`b732d35`/`88ee64b` K殻蛍光X線、
   `34a6313` 断面積のテーブル化＋補間、`e99a2b0` レイリー/コンプトン棄却
   サンプリング再設計）だった。正しい検証は「旧estimatorコード」と「新estimatorコード」を両方
   **現在のコードベース**上で動かして比較すること——これで平均値・SEM・相対誤差の
   いずれも浮動小数点誤差レベル（~1e-16）で完全一致し、置き換えが数値的に無害だと
   証明できた。チェックイン済みJSON（および`PDD_RESULTS.md`が転記している具体的な
   パーセンテージ群）は物理側の変更で既に現在のコードに対して再検証が必要な状態に
   あるが、これはEGS5側も含む別スコープの作業のため本Phaseでは着手せず、JSONは
   `git checkout`で元の内容に戻した。詳細は[[lessons_learned]]「統計不確かさ
   Phase 4: 検証スクリプトのJSON成果物は「再実行すれば裏取り完了」ではない」。
4. **文書更新**: README.mdに「統計不確かさ（相対誤差R）」節を新設（解釈ガイド・
   「必要条件であって十分条件ではない」・M<2の既定挙動を明記）、`実装済み`一覧・
   コマンド例（`--quantity relerr-dose`）も追加。CLAUDE.mdの"Known sharp edges"
   R判定追記・コマンド例更新はPhase 3時点で完了済み（本Phaseでは変更不要）。
   lessons_learned.mdに本Phaseの2件の教訓を追記。

### 受入基準の充足状況（まとめ）

1-7は Phase 0-3 で充足済み。

**8（壁時間+10%以内）は事前登録された形では判定不能**（基準の設計欠陥）。
代わりに測った同一セッション内3反復のペア比較では、条件1で統計機構ON/OFFの差が
測定分解能以下（符号が揃わない、平均−0.4%、run-to-runばらつき約2.0%）であり、
**実務上の結論としては「オーバーヘッドは無視できる」**。条件2（workers=4）は
マシンの熱ドリフト・スリープにより判定に足るデータが取れず**未決着**。
詳細はPhase 4節の(a)(b)(c)。

9（`run_chatcarlo_pdd60.py`置き換えでRESULTS.md数値が不変）は
「推定器置き換え単体では数値が一切動かない」ことを現在コードベース上での
旧/新比較で証明する形で充足——ただし`chatcarlo_pdd60_results.json`／
`PDD_RESULTS.md`自体の現在物理での再検証は別スコープの申し送り事項として残る
（下記「次のステップ」参照）。

### 次のステップ

Phase 0-4はすべて着手・完了。本計画のスコープは完了。申し送り事項:

- **`docs/egs5_crosscheck/PDD_RESULTS.md`の現在物理での再検証**: レイリー/
  コンプトン棄却サンプリング再設計（`e99a2b0`）・K殻蛍光X線実装以降、ChatCarlo
  側の物理結果は`PDD_RESULTS.md`記載時点（2026-07-15頃）から変化している。
  EGS5側の再実行も含む独立の検証タスクとして、着手時に改めて計画すること
  （本計画の対象外）。
- 「申し送り（本計画の外）」節のFOM・候補3接続・CTDI校正係数の不確かさ伝播は
  未着手のまま。
