...
!                     ****************
!
!  ChatCarlo cross-check: K-shell fluorescence, Pb slab, 100 keV pencil
!  beam, FLUORESCENCE ON (IEDGFL=1).
!
!  Built from tutorcodes/tutor5 (LATCH-based primary/Compton/Rayleigh
!  classification) + tutorcodes/tutor7 (Pb slab, 100 keV, IEDGFL flag,
!  fluorescence-photon scoring) patterns. Adds: (1) scoring of escaped
!  photons in BOTH directions (region 1 = reflected, region 3 =
!  transmitted) rather than tutor7's reflected-only spectrum, since the
!  0.05 cm slab here is thin and forward transmission dominates;
!  (2) a running total of escaped-photon energy (observable A: total
!  energy transmission fraction); (3) a 1 keV-binned escaped-photon
!  energy histogram (observable B: 72-86 keV fluorescence-peak band
!  fraction).
!
...
!    IEDGFL(2)=1  <-- fluorescence ON for this file
!    IAUGER(2) left at default 0 (Auger electrons not explicitly
!              generated; inconsequential here since electrons never
!              escape a 0.05 cm Pb slab - their energy is deposited
!              locally regardless via the EGS5 kinetic-energy cutoff)
!    Density: PEGS5 default density for element PB is 11.35 g/cm3
!              (confirmed from data/density_corrections/elements/
!              lead.density), identical to ChatCarlo's
!              materials._DENSITY_OVERRIDE for lead - no RHO override
!              needed in the ELEM &INP namelist.
!
...
!     geom passes info to our howfar routine
      ...
      common/score/count(4),entot(4),esctot,escn,ebin(120),bwidth
     ...
     *       uin,vin,win,wtin
      ...
      real*8 anorm,band7286                    ! Local variables
      integer i,j,ncase
...
! nmed and dunit default to 1, i.e. one medium and we work in cm
      ...
      chard(1) = 0.05d0      !  optional, but recommended to invoke
...
!     nreg : number of region
      ...
      med(3)=0
      med(2)=1
! Regions 1 and 3 are vacuum, region 2, Pb
...
!     slab - required in ADDITION to INCOH=1 in the PEGS5 &INP namelist
      iedgfl(2)=1
!     FLUORESCENCE ON for this run (fluor_on)
...
! defined before hatch call
      ...
      write(6,130)
130   format(/' Start fluor_on'/' Call hatch to get cross-section',
     *' data')
...
!-----------------------------------------------------------------------
      zbound=0.05d0
!     Plate is 0.05 cm (500 micron) thick
...
!  Zero LATCH-classification scoring array at start
      ...
      do i=1,120
        ebin(i)=0.0
!  Zero escaped-photon energy histogram (1 keV bins, up to 120 keV)
      end do
...
!  "primaries" -- discovered during this cross-check, see comment in
!  ausgab and the report to the user.
      iausfl(18)=1
      iausfl(20)=1
      iausfl(24)=1
...
!    Get average energies
     ...
     *i=1,4)
160   format(/' For',F6.1,' keV photons incident on',F6.3,'cm of Pb',
...
!  Observable B: fraction of escaped photons in the 72-86 keV
!  Pb K-alpha/K-beta fluorescence-peak band. bin j covers energy
!  ((j-1)*bwidth, j*bwidth] MeV, so bins 73-86 cover (0.072,0.086] MeV.
      band7286=0.0
      do j=73,86
        band7286=band7286+ebin(j)
      end do
      write(6,167) band7286, band7286/escn
167   format(' Escaped photons in 72-86 keV band (count)=',F12.1/
     *' FRACTION OF ESCAPED PHOTONS IN 72-86 keV BAND=',F10.6//)

!  Full 1-keV-binned escaped-photon energy histogram (both directions)
!  for independent/precise post-hoc re-binning if needed.
      write(6,170)
170   format(' Escaped-photon energy histogram (both directions),',
     *' 1 keV bins:'/'   bin-upper-edge(MeV)   counts')
      do j=1,120
...
!  total escaped energy, and bins it into a 1 keV energy histogram.
!
!  IMPORTANT PITFALL discovered while building this cross-check: EGS5
!  propagates LATCH from a parent photon to the fluorescence photon it
!  creates via a K/L-shell photoelectric interaction (IEDGFL=1). If we
!  only track Compton (+1) and Rayleigh (+1000) bits as tutor5 does,
!  a fluorescence photon emitted by a never-scattered ("primary")
!  parent still has LATCH=0 and gets silently miscounted as a
!  "transmitted primary" in the jj=1 bucket -- confirmed by a first
!  run where the fluorescence-ON "primary" bucket jumped from 4.3% to
!  26.6% of ncase with an average energy of 0.079 MeV (a Pb K-line
!  energy, not 0.100 MeV). Fixed here by flagging iarg=19 ("a
...
!    Total escaped-photon energy/count and spectrum (both directions)
          esctot = esctot + e(np)
          escn = escn + 1.
          ibin = min0(max0(int(e(np)/bwidth + 0.999),1),120)
...
!  We are in the Pb Slab plate - check the geometry
