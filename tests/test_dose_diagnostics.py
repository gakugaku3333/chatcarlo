...
def test_dose_max_voxel_on_water_slab_surface_triggers_background_warning():
    ...
    水側1.246e-13の僅差、n=100k時点）ようになった。解像度を細かくすると最大値が
    成長する現象自体は残るが、これはmax_substepsクランプ由来ではないと2026-07-26に
    再検証済み（accumulate_track_lengthを解析的重なり長方式に置き換えたのち、
    旧実装と直接比較して平均値が統計誤差内で一致することを確認——ビーム幅より
    ボクセルが大きい間の通常の空間平均化で、ボクセルがビーム幅を下回るとプラトーに
    収束する物理的に正しい挙動。CLAUDE.md「Known sharp edges」・
    docs/speedup_baseline/tally_exact_resolution_growth.txt参照）ため、
