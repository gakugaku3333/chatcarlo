...
- [x] CTDIvol基準の絶対線量校正（`source.ctdi_vol_mGy`。コンソール表示値を
      ...
      CTではmAs校正より汎用的。`vivemonte/ctdi.py`、mAs経路との桁の相互検証
      テスト付き）
...
- [x] 光子軌跡の3D可視化（`vivemonte trace`。既存previewテンプレートを拡張し
      軌跡なしでは従来のpreviewと同一表示になる設計。乱数を消費しない
      opt-inレコーダーで既存輸送結果に非破壊）
- [x] 線量/H*(10)マップの断面図（`vivemonte plot`。最大値ボクセルを通る
      3断面が既定、`--scene`でジオメトリー輪郭を重ねられる）
- [x] 人間と確認しながら進める進行役スキル（`.claude/skills/vive-check/`。
      ジオメトリー確認→軌跡確認→本計算→結果確認の4関門、各関門でユーザー承認）
- [ ] PHITS相互検証パイプライン（計画: [docs/plan_phits_crosscheck.md](docs/plan_phits_crosscheck.md)）
- [ ] 光電吸収時の蛍光X線を無視（局所吸収として扱っている。相互検証の後に実装予定）
